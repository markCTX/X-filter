...
def test_epel_and_remi_not_installed_fedora(Xfilter):
    '''
    ...
    epel_package = Xfilter.package('epel-release')
    assert not epel_package.is_installed
    remi_package = Xfilter.package('remi-release')
    assert not remi_package.is_installed


@pytest.mark.parametrize("tag", [('centos'), ])
def test_release_supported_version_check_centos(Xfilter):
    '''
    confirms installer exits on unsupported releases of CentOS
    '''
    # modify /etc/redhat-release to mock an unsupported CentOS release
    Xfilter.run('echo "CentOS Linux release 6.9" > /etc/redhat-release')
    distro_check = Xfilter.run('''
    source /opt/xfilter/basic-install.sh
    distro_check
    ''')
...
@pytest.mark.parametrize("tag", [('centos'), ])
def test_enable_epel_repository_centos(Xfilter):
    '''
...
@pytest.mark.parametrize("tag", [('centos'), ])
def test_php_upgrade_default_optout_centos(Xfilter):
    '''
...
@pytest.mark.parametrize("tag", [('centos'), ])
def test_php_upgrade_user_optout_centos(Xfilter):
    '''
...
@pytest.mark.parametrize("tag", [('centos'), ])
def test_php_upgrade_user_optin_centos(Xfilter):
    '''
...
@pytest.mark.parametrize("tag", [('centos'), ])
def test_php_version_lt_7_detected_upgrade_default_optout_centos(Xfilter):
    '''
    ...
    php_install = Xfilter.run('yum install -y php')
    assert php_install.rc == 0
    php_package = Xfilter.package('php')
...
@pytest.mark.parametrize("tag", [('centos'), ])
def test_php_version_lt_7_detected_upgrade_user_optout_centos(Xfilter):
    '''
...
@pytest.mark.parametrize("tag", [('centos'), ])
def test_php_version_lt_7_detected_upgrade_user_optin_centos(Xfilter):
    '''
    ...
    source /opt/xfilter/basic-install.sh
    distro_check
    install_dependent_packages XFILTER_WEB_DEPS[@]
    ''')
    expected_stdout = info_box + (' User opt-out of PHP 7 upgrade on CentOS. '
                                  'Deprecated PHP may be in use.')
    ...
    remi_package = Xfilter.package('remi-release')
    assert remi_package.is_installed
    updated_php_package = Xfilter.package('php')
